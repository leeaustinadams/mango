...
(defn sitemap
  ...
  [data-provider]
  (let [articles (mapv #(str (or (:slug %) (:_id %))) (dp/blog-articles data-provider {:status ["published"]}))
        pages (mapv #(str (or (:slug %) (:_id %))) (dp/pages data-provider {:status ["published"]}))]
    {
     :status 200
     :headers {"Content-Type" "text/plain"}
     :body (pages/sitemap (str config/site-url) articles pages)
     }))
